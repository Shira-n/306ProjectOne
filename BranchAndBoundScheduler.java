			...
			for (Node node : freeToSchedule) {
				...
				for (Processor processor : _processors) {
					...
					for (Node visited : visitedNodes){
						if (internalOrderingCheck(node, visited)){
							equivalent = true;
							break;
						}
					}
...
/////////////////////////////////////////////////////*/
	...
	/**
	 * Return true if two nodes are exchangeable, false otherwise.
	 */
	private boolean internalOrderingCheck(Node node, Node visited){

		// Check to see that the children are the same and so are the path weights to them 
		// (Not strictly true, but will do partial pruning and easier to implement)
		if (node.getChildren().keySet().size() == visited.getChildren().keySet().size()) {
			for (Node child : node.getChildren().keySet()) {
				if (visited.getChildren().keySet().contains(child) && node.getPathWeightToChild(child) == visited.getPathWeightToChild(child)) {
					;
				}
				else {
					return false;
		...
		// Check that both nodes are capable of starting at the same time on the given processor
		if (node.getParents().entrySet().size() > 0) {
			if (infulencedByParents(visited.getProcessor(), node) <= visited.getStartTime()) {
				;
			}
			else {
				return false;
